...
class BaseTool(object):
    def __init__(self):
        ...
        self.compress = [
            ".tar.gz", ".tar.bz2", ".tar.bz", ".tar.tgz", ".tar", ".tgz",
            ".zip", ".rar"
        ]
    ...
    def remove(self, filepath):
        pass
        # if os.path.exists(
        #         self.zip_to_path(filepath)) and os.path.exists(filepath):
        #     os.remove(filepath)
            ...
            try:
                ...
                if flag == "main":
                    if self.iszip(src) == ".zip":
                        ...
                        self.un_rar(src, dst)
                    elif self.iszip(src) in (".tar.gz", ".tar.bz2", ".tar.bz",
                                             ".tar.tgz", ".tar", ".tgz"):
                        if self.args.verbose:
...
def get_file_content(file_name):
    with codecs.open('./data/file_nums.txt', mode='a', encoding='utf8') as fw:
        with codecs.open(file_name, mode='r') as fr:
            content = int(fr.read())
            fw.write('{}\n'.format(content))
            # for line in fr.read():
            #     line = line.strip()
...
if __name__ == '__main__':
    ...
    """
    # 解压
    filepath = "./data/algorithm_introduction"  # relative path
    # z = UnZip(filepath, './data/output')
    # z.run()
    # 计算
    filepath = "./data/output"  # 解压后的路径
    file_names = list()
    listdir(filepath, file_names)
    ...
    for file_name in file_names:
        if file_name.endswith('gz'):
            continue
        file_count += 1
        file_number_count += get_file_content(file_name)
        # time.sleep(0.1)
    print('文件个数 ： {}'.format(file_count))
    print('内容之和 ： {}'.format(file_number_count))
